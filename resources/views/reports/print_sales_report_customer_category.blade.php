<!DOCTYPE html>
<html lang="id">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Cetak Laporan Penjualan Per Kategori Pelanggan ({{ date('d/m/Y', strtotime($dateFrom)) }} - {{ date('d/m/Y', strtotime($dateTo)) }})</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
    <style>
        body { font-family: Arial, sans-serif; font-size: 11px; color: #000; background: #fff; }
        .table-print th, .table-print td { padding: 6px 8px; border: 1px solid #ddd; }
        .table-print th { background-color: #f2f2f2 !important; text-transform: uppercase; font-size: 10px; }
        @media print {
            .no-print { display: none !important; }
            @page { size: landscape; margin: 10mm; }
        }
    </style>
</head>
<body class="p-3">
    <div class="no-print mb-3 text-end">
        <button onclick="window.print()" class="btn btn-primary btn-sm px-3"><i class="fas fa-print"></i> Cetak / Save PDF</button>
    </div>

    <div class="text-center mb-3">
        <h4 class="fw-bold mb-1">{{ strtoupper($title ?? 'LAPORAN REKAP PENJUALAN PER KATEGORI PELANGGAN') }}</h4>
        <div class="small text-muted">Periode Penjualan: {{ date('d F Y', strtotime($dateFrom)) }} s/d {{ date('d F Y', strtotime($dateTo)) }}</div>
    </div>

    <div class="row mb-3">
        <div class="col-4">
            <div class="p-2.5 border rounded text-center">
                <small class="text-muted d-block">TOTAL OMSET PENJUALAN</small>
                <strong class="fs-6 text-primary">Rp {{ number_format($grandTotalOmset, 0, ',', '.') }}</strong>
            </div>
        </div>
        <div class="col-4">
            <div class="p-2.5 border rounded text-center">
                <small class="text-muted d-block">TOTAL TRANSAKSI</small>
                <strong class="fs-6">{{ number_format($grandTotalOrders) }} Transaksi</strong>
            </div>
        </div>
        <div class="col-4">
            <div class="p-2.5 border rounded text-center">
                <small class="text-muted d-block">TOTAL ITEM TERJUAL</small>
                <strong class="fs-6 text-success">{{ number_format($grandTotalQty) }} Pcs</strong>
            </div>
        </div>
    </div>

    <table class="table table-print w-100 align-middle">
        <thead>
            <tr>
                <th style="width: 30px;">No</th>
                <th>Kategori Pelanggan (Master Customer)</th>
                <th class="text-center">Jumlah Transaksi</th>
                <th class="text-center">Total Item Terjual (Qty)</th>
                <th class="text-end">Total Omset Penjualan</th>
                <th class="text-end">Kontribusi Omset (%)</th>
            </tr>
        </thead>
        <tbody>
            @foreach($categoriesData as $idx => $row)
                @php
                    $pct = $grandTotalOmset > 0 ? ($row['total_omset'] / $grandTotalOmset) * 100 : 0;
                @endphp
                <tr>
                    <td class="text-center">{{ $idx + 1 }}</td>
                    <td class="fw-bold">{{ $row['category_label'] }}</td>
                    <td class="text-center font-monospace">{{ number_format($row['orders_count']) }}</td>
                    <td class="text-center font-monospace">{{ number_format($row['qty_sold']) }}</td>
                    <td class="text-end font-monospace fw-bold text-primary">Rp {{ number_format($row['total_omset'], 0, ',', '.') }}</td>
                    <td class="text-end font-monospace fw-bold">{{ number_format($pct, 1) }}%</td>
                </tr>
            @endforeach
        </tbody>
        <tfoot>
            <tr class="fw-bold bg-light">
                <td colspan="2" class="text-end">TOTAL REKAPITULASI:</td>
                <td class="text-center">{{ number_format($grandTotalOrders) }}</td>
                <td class="text-center">{{ number_format($grandTotalQty) }}</td>
                <td class="text-end fs-6 text-primary">Rp {{ number_format($grandTotalOmset, 0, ',', '.') }}</td>
                <td class="text-end">100.0%</td>
            </tr>
        </tfoot>
    </table>
</body>
</html>
