...
<div class="row g-3">
    ...
    <div class="col-lg-4">
        ...
        <div class="card border shadow-sm rounded-3 mb-3">
            <div class="card-header border-bottom py-2 px-3 d-flex align-items-center gap-2" style="background-color: rgba(253, 126, 20, 0.08);">
                ...
                <form action="{{ route('marketing.ads.shopee_live.start') }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="host_name" id="host_name"
                            class="form-control form-control-sm rounded-3"
                            placeholder="Contoh: Sarah Amelia" required>
                    </div>

                    <button type="submit" class="btn w-100 rounded-3 fw-bold py-2 text-white" style="background-color: #fd7e14;" @if($stores->isEmpty()) disabled @endif>
                ...
                <div class="table-responsive">
                    <table class="table table-hover align-middle mb-0" style="font-size:.8rem;">
                        ...
                        <tbody>
                            ...
                            @empty
                                <tr>
                                    <td colspan="3" class="text-center py-4 text-muted">Belum ada peringkat host Shopee.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
                            ...
                            @forelse($sessions as $session)
                                <tr>
                                    ...
                                    <td class="px-3 py-3 text-muted" style="font-size:.75rem;">
                                        ...
                                        @else
                                            <div class="text-danger fw-semibold"><i class="bi bi-clock-fill me-1 animate-pulse"></i>Sedang LIVE</div>
                                        @endif
                                    </td>
                                    <td class="px-3 py-3">
                                    ...
                                    <td class="px-3 py-3 text-end">
                                        <div class="d-flex gap-1 justify-content-end">
                                            @if($session->status === 'live')
                                                <form action="{{ route('marketing.ads.shopee_live.end', $session->id) }}" method="POST">
                                                    @csrf
                                                    <button type="submit" class="btn btn-sm btn-outline-warning px-2.5 rounded-pill fw-bold text-nowrap" style="font-size:.7rem; color: #fd7e14; border-color: #fd7e14;">
                                                        <i class="bi bi-stop-circle-fill"></i> Akhiri
                                                    </button>
                                                </form>
                                            @endif
                                            <form action="{{ route('marketing.ads.shopee_live.destroy', $session->id) }}" method="POST"
                            ...
                            @empty
                                <tr>
                                    <td colspan="7" class="text-center py-5 text-muted">
                                        <i class="bi bi-broadcast d-block fs-1 mb-2 opacity-25" style="color: #fd7e14;"></i>
                                        <div class="fw-bold text-dark small mb-1">Belum Ada Sesi LIVE Shopee Terdaftar</div>
