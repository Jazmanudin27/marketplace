    ...
    <div class="col-md-12">
        ...
        <div class="row g-3 mb-4">
            <div class="col-6 col-md-3">
                <div class="dashboard-card h-100 d-flex align-items-center gap-3 border-start border-primary border-4">
                    <div class="fs-2 text-primary opacity-75"><i class="fas fa-box"></i></div>
                    <div>
                        <h3 class="fw-bold text-white mb-0">{{ $stats['total'] }}</h3>
                        <p class="text-muted mb-0 small">Total Siap Kirim</p>
                    </div>
                </div>
            </div>
            <div class="col-6 col-md-3">
                <div class="dashboard-card h-100 d-flex align-items-center gap-3 border-start border-warning border-4">
                    <div class="fs-2 text-warning opacity-75"><i class="fas fa-clock"></i></div>
                    <div>
                        <h3 class="fw-bold text-white mb-0">{{ $stats['pending'] }}</h3>
                        <p class="text-muted mb-0 small">Belum Diproses</p>
                    </div>
                </div>
            </div>
            <div class="col-6 col-md-3">
                <div class="dashboard-card h-100 d-flex align-items-center gap-3 border-start border-4" style="border-left-color: #a855f7 !important;">
                    <div class="fs-2 opacity-75" style="color: #a855f7;"><i class="fas fa-pallet"></i></div>
                    <div>
                        <h3 class="fw-bold text-white mb-0">{{ $stats['packing'] }}</h3>
                        <p class="text-muted mb-0 small">Sedang Dikemas</p>
                    </div>
                </div>
            </div>
            <div class="col-6 col-md-3">
                <div class="dashboard-card h-100 d-flex align-items-center gap-3 border-start border-success border-4">
                    <div class="fs-2 text-success opacity-75"><i class="fas fa-check-circle"></i></div>
                    <div>
                        <h3 class="fw-bold text-white mb-0">{{ $stats['verified'] }}</h3>
        ...
        <div class="dashboard-card">
            <div class="card-header-line d-flex justify-content-between align-items-center">
                <h3 class="mb-0"><i class="fas fa-barcode"></i> Antrean Kemas Pesanan</h3>
                <div>
                ...
                <table class="data-table">
                    ...
                    <tbody>
                        @forelse($orders as $order)
                            <tr>
                                <td class="mono">
                                    <span class="fw-bold text-white">{{ $order->invoice_number ?? $order->order_marketplace_id }}</span>
                                    <div class="small text-muted mt-1">ID: {{ $order->order_marketplace_id }}</div>
                                </td>
                                <td>
                                    <div class="fw-medium">{{ $order->store->store_name }}</div>
                                    ...
                                    <ul class="ps-3 mb-0 small text-secondary-emphasis">
                                        @foreach ($order->items as $item)
                                            <li>
                                                ...
                                                {{ $item->product_name }}
                                                <span class="font-monospace bg-light bg-opacity-10 px-1 rounded small">
                                                    {{ $item->sku ?? 'No SKU' }}
                                                </span>
                                            </li>
                                        @endforeach
                                ...
                                <td>
                                    <div class="fw-medium text-white">{{ $order->courier ?? '-' }}</div>
                                ...
                                <td>
                                    @if ($order->packing_status === 'verified')
                                        <span class="badge bg-success px-3 py-2 rounded-2">
                                    ...
                                    @elseif($order->packing_status === 'packing')
                                        <span class="badge bg-warning text-dark px-3 py-2 rounded-2">
                                            <i class="fas fa-box-open"></i> Sedang Dikemas
                                        </span>
                                    @else
                                        <span class="badge bg-secondary bg-opacity-10 text-secondary border border-secondary border-opacity-10 px-3 py-2 rounded-2">
                                ...
                                <td>
                                    ...
                                    @else
                                        <a href="{{ route('fulfillment.scan_page', ['invoice' => $order->invoice_number ?? $order->order_marketplace_id]) }}"
                                            class="btn btn-primary btn-sm w-100 fw-medium">
