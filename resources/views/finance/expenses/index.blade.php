        ...
        <div class="card-body py-2 px-3">
            <form method="GET" action="{{ route('finance.expenses.index') }}">
                <div class="row g-2 align-items-end">
                    ...
                    <div class="col-6 col-md-2">
                        <label class="form-label form-label-sm fw-semibold mb-1">Kas / Rekening</label>
                        <select name="payment_source" class="form-select form-select-sm">
                            ...
                            @if(isset($bankAccounts) && $bankAccounts->isNotEmpty())
                                @foreach($bankAccounts as $bank)
                                    <option value="{{ $bank->bank_name }}" {{ $paymentSource === $bank->bank_name ? 'selected' : '' }}>{{ $bank->bank_name }}</option>
                                @endforeach
                            @else
