            ...
            <table class="shopee-table">
                <thead>
                    <tr style="border-bottom: 1px solid #e2e8f0; background-color: #f8fafc;">
                        <th class="ps-3 py-2.5" style="width: 35%;">Produk</th>
                        <th class="py-2.5 text-center" style="width: 15%;">Jumlah Pengembalian Dana</th>
                        <th class="py-2.5 text-center" style="width: 20%;">Alasan & Status</th>
                        <th class="py-2.5 text-center" style="width: 15%;">Tindakan QC / Gudang</th>
                                ...
                                <div class="d-flex flex-wrap align-items-center justify-content-between gap-3">
                                    <div class="d-flex align-items-center gap-2">
                                        ...
                                        <span class="fw-bold text-dark small">
                                            <i class="fas fa-user me-1 text-secondary text-muted"></i>{{ $ret->order->buyer_name ?? '-' }}
                                        </span>
                                    </div>
                        ...
                        <tr style="border-bottom: 1px solid #e2e8f0;">
                            ...
                            <td class="align-top py-3 px-3">
                                <div class="d-flex flex-column align-items-center">
                                    <span class="badge rounded-pill bg-danger bg-opacity-10 text-danger border border-danger-subtle px-2.5 py-1 small fw-bold">
                                        {{ strtoupper($ret->status) }}
                                    </span>
                                    
                                    <div class="small text-secondary mt-2 text-center" style="font-size: 0.76rem; max-width: 220px; line-height: 1.35;">
                            ...
                            <td class="text-center align-top py-3">
                                <div class="d-flex flex-column align-items-center justify-content-center">
                                    @if ($ret->is_restocked)
                                        <span class="badge rounded-pill bg-success bg-opacity-10 text-success border border-success-subtle px-2.5 py-1 small mb-1 fw-bold">
                                        ...
                                        @foreach($ret->items as $rItem)
                                            <div class="mt-1.5 d-flex flex-column align-items-center gap-0.5">
                                                @if ($rItem->inspection_status === 'GOOD')
                                                    <span class="badge rounded-pill bg-success bg-opacity-10 text-success border border-success-subtle px-1.5 py-0.5" style="font-size: 0.65rem;">
                                                        <i class="fas fa-check me-0.5"></i>Layak Jual
                                                    </span>
                                                @elseif ($rItem->inspection_status === 'DEFECTIVE')
                                                    <span class="badge rounded-pill bg-danger bg-opacity-10 text-danger border border-danger-subtle px-1.5 py-0.5" style="font-size: 0.65rem;">
                                                        <i class="fas fa-times me-0.5"></i>Rusak/Cacat
                                                    </span>
                                                @endif
                                        ...
                                        @endforeach
                                    @else
                                        <span class="badge rounded-pill bg-warning bg-opacity-10 text-warning border border-warning-subtle px-2.5 py-1 small mb-1 fw-bold">
                                            <i class="fas fa-hourglass-half me-1"></i>Belum QC
                                        </span>
                                        <span class="small text-muted text-center" style="font-size: 0.68rem;">Menunggu QC Fisik</span>
                                    @endif
                                </div>
                            </td>
                                    ...
                                    @if ($ret->is_restocked)
                                        ...
                                        @else
                                            <form action="{{ route('returns.replacement', $ret->id) }}" method="POST" class="w-100 d-flex justify-content-center">
                                                @csrf
                                                <button type="submit" class="btn btn-outline-primary btn-sm rounded-pill fw-semibold px-3 w-100" style="font-size: 0.72rem; max-width: 130px;" onclick="return confirm('Apakah Anda yakin ingin membuat pesanan pengganti gratis untuk retur ini?')">
                                                    <i class="fas fa-exchange-alt me-1"></i>Kirim Pengganti
                                                </button>
                                            </form>
                                        @endif
                                    @else
                                        <button type="button" class="btn btn-primary btn-sm rounded-pill px-3 fw-semibold shadow-sm w-100" style="max-width: 130px; font-size: 0.72rem;"
                                            data-bs-toggle="modal" data-bs-target="#qcModal-{{ $ret->id }}">
                                            <i class="fas fa-clipboard-check me-1"></i>Terima & QC
                                        </button>
                                    @endif
                                    <a href="{{ route('orders.show', $ret->order->id) }}" class="btn btn-outline-secondary btn-sm rounded-pill fw-semibold px-3 mt-1 w-100" style="font-size: 0.72rem; max-width: 130px;">
