    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card border shadow-sm overflow-hidden">
                <div class="card-header bg-primary bg-opacity-10 d-flex justify-content-between align-items-center border-bottom py-2 px-3">
                    <div>
                            ...
                            <div class="row g-2 align-items-end">
                                ...
                                <div class="col-12 col-sm-6 col-md-auto d-flex gap-2">
                                    ...
                                    @if (request()->anyFilled(['search', 'status']))
                                        <a href="{{ route('product_recipes.index') }}" class="btn btn-secondary btn-sm px-3">
                                ...
                                @forelse($products as $i => $p)
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <div class="text-muted font-monospace" style="font-size: 11px;">{{ $p->sku ?? '—' }}</div>
                                        </td>
                                        <td class="text-center">
                                            @if($p->is_bundle)
                                                <span class="badge bg-purple bg-opacity-10 text-purple border border-purple-subtle small px-2">Set / Bundle</span>
                                            @else
                                                <span class="badge bg-info bg-opacity-10 text-info border border-info-subtle small px-2">Single</span>
                                            @endif
                                        </td>
                                        <td>
                                            @if($recipe)
                                                <span class="fw-semibold text-secondary small"><i class="far fa-file-alt me-1"></i>{{ $recipe->name }}</span>
                                            @else
                                        ...
                                        <td class="text-end fw-bold font-monospace small">
                                            @if($recipe)
                                                <span class="text-success">Rp {{ number_format($totalCost, 0, ',', '.') }}</span>
                                            @else
                                        ...
                                        <td class="text-center">
                                            @if($recipe)
                                                <div class="btn-group btn-group-sm">
                                                    <button type="button" class="btn btn-xs btn-outline-info view-recipe-btn"
                                                    ...
                                                    <a href="{{ route('product_recipes.edit', $p->id) }}" class="btn btn-xs btn-outline-warning">
                                                        <i class="fas fa-pencil-alt me-1"></i>Edit
                                                    </a>
                                                    <form action="{{ route('product_recipes.destroy', $p->id) }}" method="POST" class="confirm-delete d-inline" data-message="Formula resep produk ini akan dinonaktifkan!">
                                                        @csrf
                                                        @method('DELETE')
                                            ...
                                            @else
                                                <a href="{{ route('product_recipes.edit', $p->id) }}" class="btn btn-xs btn-outline-primary px-3">
            ...
            <div class="modal-content overflow-hidden">
                <div class="d-flex align-items-center gap-3 p-3 border-bottom bg-info bg-opacity-10">
                    ...
                    <div class="flex-grow-1">
                        <h5 class="modal-title fw-bold fs-6 mb-0 text-dark" id="viewRecipeModalLabel">Detail Formula &amp; BOM</h5>
                ...
                <div class="modal-body p-4">
                    <div class="row g-4">
                        ...
                        <div class="col-md-7 border-end">
                            <h6 class="fw-bold text-dark mb-2"><i class="fas fa-boxes me-2 text-primary"></i>1. Bahan Baku (BOM)</h6>
                            <div class="table-responsive">
                                <table class="table table-sm table-bordered align-middle mb-0 small" id="detail-materials-table">
                                    <thead>
                        ...
                        {{-- Kanan: Jasa & QC --}}
                        <div class="col-md-5">
                            <h6 class="fw-bold text-dark mb-2"><i class="fas fa-user-cog me-2 text-warning"></i>2. Jasa &amp; QC</h6>
                            <div class="table-responsive">
                                <table class="table table-sm table-bordered align-middle mb-0 small" id="detail-labors-table">
                                    <thead>
                                        <tr class="table-light">
                                            <th>Nama Jasa / QC</th>
                    ...
                    {{-- HPP Summary Box --}}
                    <div class="card bg-success bg-opacity-10 border border-success border-opacity-20 p-3 mt-4 text-center">
                        <div class="text-muted small text-uppercase fw-bold mb-1">Estimasi Total HPP Produksi (Per 1 Pcs)</div>
        ...
        <script>
            $(document).ready(function() {
                ...
                $('.view-recipe-btn').on('click', function() {
                    ...
                    $('#detail-batch-info').text(`Dihitung berdasarkan pembagian Output Batch: ${batchQty} Pcs`);
                    ...
                    if (items.length > 0) {
                        ...
                        materialsHtml = `<tr><td colspan="4" class="text-center text-muted py-2">Tidak ada bahan baku</td></tr>`;
                    }
                    ...
                    if (labors.length > 0) {
                        ...
                        laborsHtml = `<tr><td colspan="2" class="text-center text-muted py-2">Tidak ada jasa/QC</td></tr>`;
                    }
