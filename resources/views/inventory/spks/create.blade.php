                ...
                <form action="{{ route('spks.store') }}" method="POST" enctype="multipart/form-data" class="m-0" id="spkForm">
                    ...
                    <div class="card-body p-4">
                        ...
                        <div class="border-top pt-4 mt-4">
                            ...
                            <button type="button" class="btn btn-outline-primary btn-sm w-100 fw-bold py-2 mb-3" id="btnAddRow">
    ...
    $(document).ready(function() {
        ...
        $(document).on('input focus', '.item-name', function() {
            ...
            const matches = catalogProducts.filter(function(p) {
                ...
                return name.indexOf(q) !== -1 || sku.indexOf(q) !== -1 || skuInduk.indexOf(q) !== -1;
            }).slice(0, 10);

            if (matches.length === 0) {
                $box.append('<div class="p-2 text-muted text-center italic small">Produk tidak ditemukan</div>');
            } else {
                matches.forEach(function(p) {
                    ...
                    const $item = $('<div class="p-2 border-bottom suggestion-item" style="cursor: pointer;">' +
                        ...
                        '<div class="text-muted small" style="font-size: 10px;">SKU: ' + sku + ' | Induk: ' + skuInduk + '</div>' +
                    '</div>');
                    ...
                    $item.on('click', function() {
                        ...
                        if (p.ukuran) {
                            ...
                            $sizeSelect.find('option').each(function() {
                                if ($(this).val().toUpperCase() === targetSize) {
                            ...
                            if (!sizeExists) {
                                $sizeSelect.append('<option value="' + p.ukuran + '">' + p.ukuran + '</option>');
