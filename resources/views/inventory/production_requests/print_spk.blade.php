...
<head>
    <meta charset="UTF-8">
    <title>SPK Surat Perintah Kerja - {{ $productionRequest->request_number }}</title>
    <style>
...
<body>
    ...
    <table class="header-table">
        <tr>
            <td class="header-left">
                <div><span class="info-label">NO PRODUKSI :</span> <span style="font-family:monospace; font-weight:bold;">{{ $productionRequest->request_number }}</span></div>
                <div style="margin-top: 3px;"><span class="info-label">NO PESANAN :</span> <span style="font-weight:bold;">{{ $productionRequest->request_type === 'po' ? 'PO CUSTOM' : 'STOCK GUDANG' }}</span></div>
            </td>
    ...
    <div class="design-box">
        ...
        <div class="design-image-container">
            @php $imageShown = false; @endphp
            @foreach($productionRequest->items as $item)
                @if($item->masterProduct && $item->masterProduct->image_url && !$imageShown)
                    <img class="design-image" src="{{ $item->masterProduct->image_url }}" alt="Desain">
                    @php $imageShown = true; @endphp
                @endif
            @endforeach
            
            @if(!$imageShown)
                <div style="color:#777; font-size:10px; border:1px dashed #ccc; padding: 25px 40px; border-radius: 4px; background:#fafafa;">
                    <i class="fas fa-image fa-2x d-block mb-1 text-muted" style="font-size:24px;"></i>
    ...
    @php
        $warnaList = [];
        foreach($productionRequest->items as $item) {
            if($item->masterProduct && $item->masterProduct->warna) {
                $warnaList[] = $item->masterProduct->warna;
            }
        }
        $warnaString = !empty($warnaList) ? implode(', ', array_unique($warnaList)) : 'UMUM';
    @endphp
    <div class="table-title">Arsip Kantor: Rincian Produk ({{ strtoupper($warnaString) }})</div>
    ...
    <table class="product-table">
        <thead>
            <tr>
                ...
                <th rowspan="2" style="width: 12%;">Total QTY</th>
                <th rowspan="2" style="width: 25%;">Barang Sudah Ada Di Kantor</th>
            </tr>
            <tr>
        ...
        <tbody>
            ...
            @foreach($grouped as $model)
                <tr>
                    ...
                    @foreach($sizesHeader as $sz)
                        <td>{{ $model['sizes'][$sz] > 0 ? $model['sizes'][$sz] : '' }}</td>
                    @endforeach
    ...
    <div class="attrib-box">
        ...
        <div class="attrib-content">
            BORDIR-LOGO : {{ $grandTotal }} Pcs
            @if($productionRequest->shipping_address)
                <br><span style="font-weight:normal; font-size:10px; color:#555; margin-top:4px; display:block;">Alamat Kirim: {{ $productionRequest->shipping_address }}</span>
            @endif
        </div>
    </div>
    ...
    <div class="doc-checklist">
        ...
        <div class="doc-item">
            <span class="checkbox-square"></span> SUDAP FOTO
        </div>
        <div class="doc-item">
