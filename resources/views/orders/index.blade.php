...
@section('content')
    ...
    <style>
        ...
        .badge-tokopedia {
            background: linear-gradient(135deg, #03ac0e, #10b981);
            color: #fff;
        }
            ...
            <div class="shopee-tabs" role="tablist">
                @foreach ($tabStatuses as $tabKey => $tabInfo)
                    @php
                        ...
                        $badgeStyle =
                            $tabKey === 'READY_TO_SHIP' && $count > 0
                                ? 'background:#ee4d2d; color:#fff;'
                                : ($count > 0
                                    ? 'background:#ee4d2d; color:#fff;'
                                    : 'background:#e5e7eb; color:#888;');
                    @endphp
                    <a class="shopee-tab {{ $isActive ? 'active' : '' }}" href="{{ $tabUrl }}" role="tab">
                        ...
                        <div class="col-12 col-md-3">
                            <div class="shopee-filter-group w-100">
                                ...
                                <input type="text" name="order_number" class="form-control w-100"
                                    placeholder="Cari no. pesanan, resi, pembeli..." value="{{ request('order_number') }}">
                            </div>
                        </div>
                                ...
                                <select name="store_id" class="form-select w-100">
                                    ...
                                    @foreach ($stores as $store)
                                        @php
                                            $channelName =
                                                $store->channel->name ??
                                                ucfirst($store->channel->code ?? 'Marketplace');
                                        @endphp
                            ...
                            @forelse($orders as $order)
                                @php
                                    ...
                                    $statusBadgeClass = match (true) {
                                        ...
                                        in_array($orderStatusUp, ['READY_TO_SHIP', 'TO_SHIP', 'PROCESSED'])
                                            => 'status-toship',
                                        in_array($orderStatusUp, ['SHIPPED', 'IN_TRANSIT', 'TO_RECEIVE'])
                                            => 'status-shipped',
                                        in_array($orderStatusUp, ['COMPLETED', 'FINISHED', 'SELESAI', 'DELIVERED'])
                                            => 'status-completed',
                                        in_array($orderStatusUp, ['CANCELLED', 'BATAL', 'IN_CANCEL'])
                                            => 'status-cancelled',
                                ...
                                <tr>
                                            ...
                                            <div class="d-flex flex-wrap gap-1 align-items-center mt-1">
                                                @if ($order->is_dropship)
                                                    <span class="meta-badge"
                                                        style="background:#fff7e6;color:#d46b08;border:1px solid #ffd591;">Dropship</span>
                                                @endif
                                                ...
                                                @if ($order->spks && $order->spks->isNotEmpty())
                                                    <a href="{{ route('spks.show', $order->spks->first()->id) }}"
                                                        ...
                                                        <i
                                                            class="fas fa-tools me-1"></i>{{ $order->spks->first()->no_spk }}
                                                    </a>
                                                @else
                                                    <span class="meta-badge"
                                                        style="background:#fafafa;color:#888;border:1px solid #ddd;">
                                    ...
                                    <td style="text-align:right;">
                                        <div
                                            style="font-size:0.85rem; font-weight:700; color:#333; font-family:'Courier New',monospace;">
                                    ...
                                    <td>
                                        <div style="font-size:0.78rem; color:#555;">
                                            <div style="margin-bottom:3px;">
                                                ...
                                                <span
                                                    style="font-weight:600;">{{ $order->order_date ? $order->order_date->format('d/m/Y H:i') : '-' }}</span>
                                            </div>
                                            @if ($order->ship_before_date)
                                                ...
                                                @if (
                                                    ...
                                                    @if ($order->is_ship_overdue)
                                                        <span class="deadline-overdue"><i
                                                                class="bi bi-exclamation-circle me-1"></i>Overdue</span>
                                                    @elseif ($order->is_ship_urgent)
                                                        <span class="deadline-urgent"><i
                                                                class="bi bi-clock me-1"></i>{{ $order->ship_before_date->diffForHumans() }}</span>
                                                    @else
                                                        <span class="deadline-safe"><i
                                                                class="bi bi-check-circle me-1"></i>{{ $order->ship_before_date->diffForHumans() }}</span>
                                                    @endif
                                                @endif
                                            @endif
                                            @if ($order->completed_at && in_array($orderStatusUp, ['COMPLETED', 'FINISHED', 'SELESAI', 'DELIVERED']))
                                                <div
                                                    style="margin-top:3px; font-size:0.7rem; color:#15803d; font-weight:600;">
                                                    <i class="fas fa-check-double me-1"></i>Cair:
                                                    <span
                                                        style="font-family:monospace;">{{ $order->completed_at->format('d/m/Y H:i') }}</span>
                                                </div>
                                            @endif
                                        </div>
                                    </td>
                                        ...
                                        <div class="d-flex flex-column align-items-center gap-1">
                                            ...
                                            @if ($order->order_status === 'CANCELLED' && $order->cancel_reason)
                                                <div class="text-danger text-truncate"
                                                    style="max-width:110px; font-size:0.62rem;"
                                            ...
                                            @if ($order->order_status !== 'CANCELLED')
                                                ...
                                                @elseif($order->packing_status === 'packing')
                                                    <span class="meta-badge"
                                                        style="background:#fffbe6;color:#d4b106;border:1px solid #ffe58f;">
                                                        <i class="fas fa-box-open me-1"></i>Packing
                                                    </span>
                                                @else
                                                    <span class="meta-badge"
                                                        style="background:#fafafa;color:#aaa;border:1px solid #eee;">
                                ...
                                <tr>
                                    <td colspan="9">
                                        <div class="shopee-empty">
                                            ...
                                            <p style="font-size:0.78rem; color:#bbb; margin-top:4px;">Coba ubah filter atau
                                                tab status di atas.</p>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
        ...
        document.addEventListener('DOMContentLoaded', function() {
                            ...
                            if (data.success) {
                                ...
                                if (typeof Swal !== 'undefined') {
                                    Swal.fire({
                                        ...
                                        text: data.message ||
                                            'Terjadi kesalahan saat menarik resi.',
                            ...
                            fetch(`{{ url('/orders') }}/${orderId}/ship`, {
                                ...
                                .then(data => {
                                    if (data.success) {
                                        ...
                                        const trackingBtn = $flexContainer ?
                                            $flexContainer.querySelector(
                                                '.btn-fetch-single-tracking') : null;
                                        if (data.tracking_number && trackingBtn) {
                                            const trackingDiv = trackingBtn.closest(
                                                'div');
                                            if (trackingDiv) {
            ...
            if (btnShip) {
                btnShip.addEventListener('click', function(e) {
                    ...
                    if (checked.length === 0) {
                        Swal.fire('Pilih Pesanan', 'Pilih minimal satu pesanan dengan mencentang checkbox.',
                            'warning');
                        return;
                    }
                    Swal.fire({
