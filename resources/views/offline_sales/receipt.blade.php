...
<head>
    ...
    <link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;600&family=Outfit:wght@600&display=swap"
        rel="stylesheet">
    <style>
        * {
            margin: 0;
    ...
    <div class="receipt">
        ...
        @if ($offlineSale->customer && $offlineSale->customer->address)
            <div class="row" style="flex-direction: column; align-items: flex-start; margin-top: 4px;">
                <span>Alamat:</span>
                <span
                    style="font-size: 11px; color: #555; text-align: left; width: 100%; white-space: normal; word-break: break-word;">{{ $offlineSale->customer->address }}</span>
            </div>
        @endif
        @if ($offlineSale->is_dropship)
            <div style="border-top: 1px dashed #777; margin: 6px 0; padding-top: 6px;">
                <div class="center bold" style="font-size: 10px; margin-bottom: 4px; text-transform: uppercase;">Kirim
                    Sebagai Dropshipper</div>
                <div class="row"><span>Pengirim</span><span
                        class="bold">{{ $offlineSale->dropshipper_name }}</span></div>
        ...
        <table class="items-table">
            @foreach ($offlineSale->items as $item)
                ...
                <tr>
                    <td colspan="2" style="color:#555;font-size:11px;padding-top:0;">@ Rp
        ...
        <div class="row"><span>Subtotal</span><span>Rp
                {{ number_format($offlineSale->total_amount, 0, ',', '.') }}</span></div>
        @if ($offlineSale->discount_amount > 0)
            <div class="row"><span>Diskon</span><span>- Rp
                    {{ number_format($offlineSale->discount_amount, 0, ',', '.') }}</span></div>
        @endif
        <div class="divider"></div>
        <div class="total-row">
            <span>TOTAL</span>
        ...
        <div class="row"><span>{{ $offlineSale->payment_method_label }}</span><span>Rp
                {{ number_format($offlineSale->paid_amount, 0, ',', '.') }}</span></div>
        <div class="row bold"><span>Kembalian</span><span>Rp
                {{ number_format($offlineSale->change_amount, 0, ',', '.') }}</span></div>
